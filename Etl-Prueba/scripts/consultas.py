...
def temperatura_promedio_por_ciudad():
    ...
    print("\n[INFO] TEMPERATURA PROMEDIO POR CIUDAD:")
...
def ciudad_mas_humeda():
    ...
    if registro:
        print(f"\n[INFO] CIUDAD MAS HUMEDA: {registro.nombre} con {registro.humedad}%")

def velocidad_viento_max():
    """Velocidad maxima de viento registrada"""
    registro = db.query(
        Ciudad.nombre,
    ...
    if registro:
        print(f"\n[INFO] VIENTO MAS FUERTE: {registro.nombre} con {registro.velocidad_viento} km/h")
...
if __name__ == "__main__":
    try:
        print("\n" + "="*50)
        print("ANALISIS DE DATOS - POSTGRESQL")
        print("="*50)
